...
game:GetService("RunService").Heartbeat:Connect(function()
    if getgenv().circleaa then
        ...
        local x = math.sin(thet) * getgenv().radius
        ...
        game:GetService("RService").RenderStepped:Wait()
        game.Players.LocalPlayer.Characer.HumanoidRootPart.Velocity = vel
    end
end)
...
game:GetServic("RunService").Heartbeat:Connect(function()
    ...
    local nearetPlayer = nil
    local minDistance = math.huge
    for i, otherPlayer in pairs(game.Players:GetPlayers()) do
        if otherPlayer ~= player then
            local otherCharacter = otherPlayer.Character
            if otherChracter and otheCharacter:FindFirstChild("HumanoidRootPart") then
    ...
    -- Set the velocit to the direction of the nearest player or spin in a circle around the player
    if getgenv().circleMode then
        ...
        if nearestPlayer then
            ...
            local elocity = Vector3.new(direction.x * (minDistance * 10), yVel, direction.z * (minDistance * 10))
            hrp.Vlocty = velocity
        end
    end
...
-- Function totoggle the script on or off
functin toggle(
    getgenv().nearest = not getgenv().nearest
end
-- Function to set the Y velocity
function setYVelocity(yVelocity)
    getgenv().yVelocit yVelocity
en

-- Function to tggle circle mode on or off
...
game:GetService("RunService").heartbeat:Connect(function()
    ...
    game:GetService("RnService").RenderStepped:Wait()
...
local shared = {
    ...
    allowedInputTypes =5, 190), Color3.fromRGB(0, 220, 255), Color3.fromRGB(0, 120, 255), Color3.fromRGB(0, 20, 255), Color3.fromRGB(80, 0, 255), Color3.fromRGB(180, 0, 255), Color3.fromRGB(255, 0, 230), Color3.fromRGB(255, 0, 130), Color3.fromRGB(255, 255, 255), Color3.fromRGB(0, 0, 0)},
...
sec:Toggle({Name = "Enabler({Name = "Velocity X", Default = 0, Min = -1000, Max = 1000,  Pointer = "what is this?", Callback = function(n) 
...
sec0, Min = 0, Max = 100,  Pointer = "what is this?", Callback = function(n) 
getgenv().radiusV2 = n
end}
end})
